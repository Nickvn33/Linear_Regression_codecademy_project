...
states_sample <- sample(c(TRUE, FALSE), nrow(state_df), replace = T, prob = c(0.6, 0.4))
states_train <- state_df[states_sample, ]
states_test <- state_df[!states_sample, ]
```
...
states_model <- lm(Income ~ Illiteracy, data = states_train)
```
...
summary(states_model)$r.squared
```
...
states_train$estimate <- predict(states_model)
states_train$residuals <- residuals(states_model)
...
ggplot(states_train, aes(Illiteracy, Income)) + 
  geom_point() + 
  geom_point(aes(y = estimate), color = "blue") +
...
state_model_multi <- lm(Income ~ `HS Grad` + Population, data = states_train)
summary(state_model_multi)
```
